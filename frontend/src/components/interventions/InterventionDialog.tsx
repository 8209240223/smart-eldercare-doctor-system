...
import type { Intervention } from "@/hooks/useApi";
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto rounded-2xl border-border/40 bg-white/95 p-0 backdrop-blur-xl">
        ...
        <form onSubmit={submit} className="grid grid-cols-1 gap-4 p-6 md:grid-cols-2">
          <div className="space-y-2"><Label>老人 *</Label><select required value={form.elderId || ""} onChange={(event) => update("elderId", Number(event.target.value))} className="h-9 w-full rounded-md border border-input bg-transparent px-3 text-sm"><option value="">请选择老人</option>{elders.map((elder) => <option key={elder.id} value={elder.id}>{elder.name}</option>)}</select></div>
          <div className="space-y-2"><Label>关联随访记录ID</Label><Input type="number" min={1} value={form.followRecordId || ""} onChange={(event) => update("followRecordId", event.target.value ? Number(event.target.value) : undefined)} /></div>
