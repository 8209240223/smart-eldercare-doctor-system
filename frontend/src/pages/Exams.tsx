...
export default function Exams() {
  ...
  const canEditExams = role === "doctor" || role === "nurse";
  const canDeleteExams = role === "doctor";
  ...
  // 对比分析跟随左侧选中的老人(不再用独立下拉)
  const { data: compareData, isLoading: compareLoading } = useExamCompare(elderId);
  ...
  // 选中老人后自动把上面的日期填成该老人体检记录的最早/最晚日期
  useEffect(() => {
    if (!elderId || !compareData || compareData.length === 0) return;
    const dates = compareData
